...
export default function DroneModel({ scrollProgress = 0 }: { scrollProgress?: number }) {
    ...
    useEffect(() => {
        ...
        // ── MATERIALS ──────────────────────────────────────────────
        const hull = new THREE.MeshStandardMaterial({ color: 0x242435, metalness: 0.8, roughness: 0.2 });
        const hullDark = new THREE.MeshStandardMaterial({ color: 0x161625, metalness: 0.85, roughness: 0.15 });
        const hullMid = new THREE.MeshStandardMaterial({ color: 0x2a2a4a, metalness: 0.7, roughness: 0.3 });
        const chrome = new THREE.MeshStandardMaterial({ color: 0x667799, metalness: 0.98, roughness: 0.05 });
        const panelMat = new THREE.MeshStandardMaterial({ color: 0x1a1a2e, metalness: 0.8, roughness: 0.4 });
        const legMat = new THREE.MeshStandardMaterial({ color: 0x28283a, metalness: 0.85, roughness: 0.25 });
        const jointMat = new THREE.MeshStandardMaterial({ color: 0x4a5a8a, metalness: 0.95, roughness: 0.1 });
        const ventMat = new THREE.MeshStandardMaterial({ color: 0x0f0f1a, metalness: 0.6, roughness: 0.6 });
        ...
        drone.scale.set(1.25, 1.25, 1.25); // Increased size by a little bit
        scene.add(drone);
        ...
        // ─────────────────────────────────────────────────────────
        // Realistic High-Speed Fan Assembly
        // ─────────────────────────────────────────────────────────
        const fanGroup = new THREE.Group();
        fanGroup.position.set(0, 0, 0.98);
        eye.add(fanGroup);

        // 1. Motion Blur Disc (semi-transparent disc behind blades)
        ...
        add(new THREE.CircleGeometry(0.44, 32), blurMat, fanGroup, 0, 0, -0.005);

        // 2. Turbine Blades (12 tapered blades)
        ...
        for (let b = 0; b < 12; b++) {
            ...
            // Tapered blade shape
            const bladeGeo = new THREE.CylinderGeometry(0.12, 0.42, 0.08, 4, 1, true);
            const bm = new THREE.Mesh(bladeGeo, bladeMat);
            bm.position.set(0, 0.24, 0);
            bm.rotation.x = Math.PI / 2;
            bm.rotation.z = 0.5; // Pitch
            bm.scale.set(0.1, 1, 0.4);
            bg.add(bm);

            // Edge highlight rim on each blade
            add(new THREE.BoxGeometry(0.005, 0.38, 0.01), chrome, bm, 0, 0, 0.04);
        }

        // 3. Central fan hub
        add(new THREE.SphereGeometry(0.08, 16, 16), hullDark, fanGroup, 0, 0, 0.01);
        add(new THREE.TorusGeometry(0.082, 0.008, 8, 32), chrome, fanGroup, 0, 0, 0.02);
        ...
        // 8. LIGHTS
        // ─────────────────────────────────────────────────────────
        const keyL = new THREE.DirectionalLight(0xc8d8ff, 8); // Boosted
        keyL.position.set(5, 8, 5); keyL.castShadow = true;
        keyL.shadow.mapSize.set(2048, 2048); scene.add(keyL);

        const fillL = new THREE.DirectionalLight(0xffe8cc, 4); // Boosted
        fillL.position.set(-5, 2, -3); scene.add(fillL);

        const rimL = new THREE.DirectionalLight(0xffffff, 10); // Much stronger rim light
        rimL.position.set(0, -3, -8); scene.add(rimL);

        const sideRim = new THREE.DirectionalLight(0x88aaff, 6); // Extra rim light for contour
        sideRim.position.set(8, 0, -5); scene.add(sideRim);

        const topL = new THREE.DirectionalLight(0x88aaff, 5); // Boosted
        topL.position.set(0, 10, 0); scene.add(topL);

        // Add HemisphereLight for better ambient environment response
        const hemiL = new THREE.HemisphereLight(0x88aaff, 0x000000, 4);
        scene.add(hemiL);

        scene.add(new THREE.AmbientLight(0x202044, 4)); // Lightened and boosted ambient
    ...
    return (
        <div ref={mountRef} style={{ width: '100%', height: '100%', position: 'absolute', inset: 0, touchAction: 'none' }} />
    );
}
